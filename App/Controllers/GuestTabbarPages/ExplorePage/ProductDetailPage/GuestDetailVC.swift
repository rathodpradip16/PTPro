...
//        cell.instantView.frame = CGRect(x:cell.entirehomeLabel.frame.origin.x+cell.entirehomeLabel.frame.size.width+3, y: 172, width: 10, height: 12)
        ...
        return cell
            ...
            if !(self.propertyReviewArray[indexPath.row].isAdmin ?? true){
                ...
                let attributedString = NSMutableAttributedString(string: "\(Utility.shared.getLanguage()?.value(forKey: "Verifiedby") ?? "Verified by") \(Utility.shared.getLanguage()?.value(forKey: "appname") ?? "\(appName)")" , attributes: attributes as [NSAttributedString.Key : Any])
